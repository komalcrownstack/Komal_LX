...
Cypress.Commands.add('suplogout', () => {
  cy.get('.h-8').should('be.visible').click();
  cy.wait(4000)
  cy.get('#headlessui-menu-item-6 > div').should('be.visible').click();
  cy.get('.mt-3 > .mt-2 > div').should('have.text', 'Are you sure, you want to sign out?').and('be.visible');
  cy.get('.border > div').should('be.visible');
  cy.xpath("//button//div[text()='Confirm']").should('be.visible').click();
  cy.wait(5000)
...
//  cy.xpath('//input[@name="phone"]').should('be.visible').clear().type(phone)
  ...
  cy.xpath('//input[@name="postalCode"]').should('be.visible').type('85006')
  ...
  //cy.wait(2000)
  //cy.xpath('//*[@class=" css-yt9ioa-option"][contains(text(),"PHOENIX")]').scrollIntoView().click()
  cy.wait(4000)
