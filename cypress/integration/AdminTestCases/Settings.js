...
describe("Change password Testcases", () => {
    ...
    beforeEach(() => {
        cy.login();
        cy.xpath("//nav//div[text()='Settings']").scrollIntoView().should('be.visible').should('contain','Settings')
    ...
    it("As a Admin I should be able to navigate on change password screen", () => {
        ...
        LoginPages.GetURL().should('include', '/password');
    })
    it("At Chnage Password page each page should have proper labels", () => {
        LoginPages.GetPageScroll().scrollIntoView().should('be.visible').should('contain','Settings')
        LoginPages.GetPageScroll().should('be.visible').click()
        //Verifing the "Change Password" Header is present.
        LoginPages.GetChangePassword().should('be.visible').and('have.text', 'Change Password');
        //Verifying the label of "Old Password" input field.
        LoginPages.GetOddPassword().eq(0).should('be.visible').and('have.text', 'Old Password');
        //Verifying that the "Old Password" input field is present.
        LoginPages.GetOddPasswordVisiable().should('be.visible')
        //Verifying the label of "New Password" input field.
        LoginPages.GetOddPassword().eq(1).should('be.visible').and('have.text', 'New Password');
        //Verifying that the "New Password" input field is present.
        LoginPages.GetNewPassword().should('be.visible')
        //Verifying the label of "Confirm Password" input field.
        LoginPages.GetOddPassword().eq(2).should('be.visible').and('have.text', 'Confirm Password');
        //Verifying that the "Confirm Password" input field is present.
        LoginPages.GetConfirmPassword().should('be.visible')
        //Verifying the label of "Update" button.
        LoginPages.GetUpdatePassword().should('be.visible').and('have.text', 'Update');
    })

    it('Validate "Change Password" section with Blank Data.', () => {
        //Clicking on Submit button with blank data
        LoginPages.GetUpdatePassword().should('be.visible').click();
        //Verfying the validation error for Blank Data for "Old Password" input field.
        LoginPages.GetOldPasswordValidation().should('be.visible');
        //Verfying the validation error for Blank Data for "Old Password" input field.
        LoginPages.GetOldPasswordFiled().should('be.visible');
        //Verfying the validation error for Blank Data for "Old Password" input field.
        LoginPages.
        GetOldPasswordFiled().should('be.visible');
    })
    ...
    it("Validate 'Change Password' section with Minimum Characters.", () => {
        ...
        LoginPages.Get8CharacterLabel().eq(0).should('have.text', 'Password is too short - should be 8 chars minimum').and('be.visible');
        //Verfying the validation error for minimum characters for "New Password" input field.
        LoginPages.Get8CharacterLabel().eq(1).should('have.text', 'Password should be atleast 8 characters and should contain combination of uppercase, lowercase & numbers').and('be.visible');
        //Verfying the validation error for minimum characters for "New Password" input field.
        LoginPages.Get8CharacterLabel().eq(2).should('have.text', 'Password should be atleast 8 characters and should contain combination of uppercase, lowercase & numbers').and('be.visible');
    })
    ...
    it("Validate 'Change Password' section if both old, New Passwords are same also when both New and confirm password are not matching.", () => {
        ...
        LoginPages.Get8CharacterLabel().eq(0).should('have.text', 'Old and new password can not be the same').and('be.visible');
        //Verfying the validation error if both Confirm and New passwords are not same.
        LoginPages.Get8CharacterLabel().eq(1).should('have.text', 'Both passwords need to be the same').and('be.visible');
    })
    ...
    it("As a Admin I should be able to change password and admin should be able to login with new password", () => {
        ...
        LoginPages.GetUpdatePassword().click();
        cy.wait(2000);
        ...
        LoginPages.GetPasswordUpdateSuccessfully().should('be.visible');
        //Entering valid data into "Email" input field.
        LoginPages.GetInputEmial().type(cred.email)
        //Entering valid data into "Password" input field.
        LoginPages.GetInputPassword().type('Passcode@123')
        //Clicking on "Sign in" button.
        LoginPages.GetSignIN().click()
        cy.wait(2000);
        ...
        LoginPages.GetURL().should('include', '/dashboard');
